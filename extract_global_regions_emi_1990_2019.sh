...
cdo expr,PMOA=emi_LIP+emi_POL+emi_PRO ${fnm}_emi.nc tmp_emi0.nc
...
cdo -mulc,1E-09 -mulc,86400 -fldsum -mul tmp_${region}.nc tmp_area_${region}.nc tmp_${region}_emi_glb_${dat}.nc

#- convert unit (kg m-2 s-1 --> ug m-2 s-1)
cdo -mulc,1E09 -setunit,'ug m-2 s-1' tmp_${region}.nc tmp_${region}_emi_whole_grid_glb_${dat}.nc
...
for (( year=2009; year<=2019; year+=1 )); do
       echo $year
       cdo -yearsum -mergetime tmp_${region}_emi_glb_${year}*.nc tmp_emi_glb_yr_${year}.nc       

done


#- compute monthly mean, merge to one time series, compute yearly mean, average over all years
cdo -timmean -mergetime tmp_emi_glb_yr*.nc  PMOA_${region}_emi_mean_glb_annual_total_1990_2019.nc
cdo -timmean  -yearmean -mergetime [ -apply,-monmean [ tmp_${region}_emi_whole_grid_glb_*.nc ] ] PMOA_${region}_emi_mean_whole_grid_glb_annual_total_1990_2019.nc
