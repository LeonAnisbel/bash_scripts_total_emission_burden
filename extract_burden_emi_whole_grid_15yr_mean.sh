...
exp="ac3_arctic"
liste=$(ls -d ${base_path}/${exp}/${exp}_*.01_emi.nc)
var="emi_POL"
region="global"
rm tmp_*
folder_1="1990-2004"
...
#cdo expr,emi_PMOA=emi_POL+emi_LIP+emi_PRO ${fnm}_emi.nc tmp_emi0.nc
...
#- compute area-weighted total across the globe (kg m-2 s-1 --> kg yr-1), convert unit (kg yr-1 --> Tg yr-1)
cdo -mulc,1E-09 -mulc,86400 -fldsum -mul tmp_emi.nc tmp_area.nc tmp_${region}_emi_glb_${dat}.nc

#- convert unit (kg m-2 s-1 --> mg m-2 yr-1)
cdo -mulc,1E06 -mulc,86400 -setunit,'mg m-2 yr-1' tmp_emi.nc tmp_emi_whole_grid_glb_${dat}.nc

done

#file_to_group='tmp_'${region}'_emi_glb'
#file_to_group1='tmp_emi_glb_yr'
#file_to_group2='annual'
file_to_group='tmp_emi_whole_grid_glb'
file_to_group1='tmp_emi_whole_grid_yr'
file_to_group2='whole_grid_glb_annual_total'
...
for (( year=2005; year<=2019; year+=1 )); do
       echo $year
       cdo -yearsum -mergetime ${folder_2}/${file_to_group}*.nc ${folder_2}/${file_to_group1}_${year}.nc
done
cdo -timmean -mergetime ${folder_2}/${file_to_group1}*.nc  ${var}_emi_mean_${file_to_group2}_2005_2019.nc



for (( year=1990; year<=2004; year+=1 )); do
       echo $year
       cdo -yearsum -mergetime ${folder_1}/${file_to_group}*.nc ${folder_1}/${file_to_group1}_${year}.nc  #${var}_emi_mean_${region}_annual_2005_2019.nc
done
cdo -timmean -mergetime ${folder_1}/${file_to_group1}*.nc  ${var}_emi_mean_${file_to_group2}_1990_2004.nc


for (( year=1990; year<=2019; year+=1 )); do
       echo $year
       cdo -yearsum -mergetime ${file_to_group}*.nc ${file_to_group1}_${year}.nc  #${var}_emi_mean_${region}_annual_2005_2019.nc
done
cdo -timmean -mergetime ${file_to_group1}*.nc  ${var}_emi_mean_${file_to_group2}_1990_2019.nc
